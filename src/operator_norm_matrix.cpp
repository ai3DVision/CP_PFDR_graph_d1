 ...
 * returns the square operator norm of the matrix */
{
    ...
    if (P == 0){
        ...
        symmetrized = 1;
        const int ntMNP = compute_num_threads(M*N*P/2, P);
        const int ntP = compute_num_threads(P, P - 1);
        ...
        if (P == M){ /* case A A^t */
            ...
            #pragma omp parallel for private(m, n, p, Am, An, Ap, a) \
                schedule(static) num_threads(ntMNP)
            for (p = 0; p < P; p++){
                Am = A;
                An = A + p;
                Ap = AA + P*p;
        ...
        #pragma omp parallel for private(m, n, p, Ap) schedule(static) \
            num_threads(ntP)
        for (p = 0; p < P - 1; p++){
            Ap = AA + P*p;
            m = P + (P+1)*p;
    ...
    n2 = ZERO;
    #pragma omp parallel private(X, AX, i, n, a, b, seed) reduction(max:n2) \
        num_threads(ntI)
    {
