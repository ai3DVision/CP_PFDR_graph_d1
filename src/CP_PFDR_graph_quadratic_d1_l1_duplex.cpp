    ...
    Graph<real, real, real> **G, int **Vc, int **rVc, real **R, real *obj)
{
    ...
    const int ntV = compute_num_threads(V, N);
    const int ntNV = compute_num_threads(N*V, N);
    const int ntVV = compute_num_threads(V*V, V);

    /**  construct graph **/
    Graph<real, real, real> *H;
    if (La_l1 == NULL && !positivity){ //differentiable case -> one level graph LOIC
        H = new Graph<real, real, real>(V, E);
        H->add_node(V);
        /* d1 edges */
        for (e = 0; e < E; e++){
            H->add_edge(Eu[e], Ev[e], ZERO, ZERO);
        }
        /* source/sink edges */
        for (v = 0; v < V; v++){
            H->add_tweights(v, ZERO, ZERO);
        }
    }
    else {//non-differentiable case -> two levels graph LOIC
        H = new Graph<real, real, real>(2 * V, 2 * E + V);
        H->add_node(2 * V);
        /* d1 edges */
        for (e = 0; e < E; e++){
            H->add_edge(Eu[e], Ev[e], ZERO, ZERO); //(4 * e): u1->v1; (4 * e + 1) :  u1<-v1
            H->add_edge(Eu[e] + V, Ev[e] + V, ZERO, ZERO); //(4 * e + 2): u2->v2; (4 * e + 3) :  u2<-v2
        }
        /* source/sink edges */
        for (v = 0; v < V; v++){
            H->add_tweights(v, ZERO, ZERO); //source to v1
            H->add_tweights(v + V, ZERO, ZERO); //v2 to sink
            H->add_edge(v, v + V, ZERO, ZERO); //v1 to v2 //(4 * E + 2 * v): u1->u2; (4 * E + 2 * v + 1) :  u1<-u2
        }
    }
    *G = H;
...
/* 26 arguments */
{
    ...
    real a, b, c, d, m; /* general purpose temporary real scalars - LOIC*/
    ...
    const int ntVN = compute_num_threads(N*V, N);
    const int ntVV = compute_num_threads(V*V, V);
    ...
    while (true){
        ...
        if (N > 0){ /* direct matricial case, DfS = -(A^t) R */
            #pragma omp parallel for private(v, Av, a, n) num_threads(ntVN)
            for (v = 0; v < V; v++){
                Av = A + N*v;
                a = ZERO;
        ...
        for (u = 0; u < V; u++){
            for (ee = G->nodes[u].first; ee; ee = ee->next){
                if (ee->is_active){ //only first level u1 - v1 edges can be active - LOIC
                    v = (int) (ee->head - G->nodes);
                    if (v > V) {std::cout << "SHOULD NEVER HAPPEN 1" << std::endl;}  //LOIC
                    e = ((int) (ee - G->arcs))/4; //LOIC
                    d = (*rX)[Cv[u]] - (*rX)[Cv[v]];
                    if (d > ZERO){
                        DfS[u] += La_d1[e];
                    }else if (d < ZERO){
        ...
        if (La_l1 == NULL && !positivity){ /* differentiable case */
            std::cout << "DIFFERENTIABLE CASE" << std::endl;
            ...
            real * DfS_up = (real*) malloc(V*sizeof(real)); // directional derivative in upward direction - LOIC
            real * DfS_do = (real*) malloc(V*sizeof(real)); // directional derivative in downward direction - LOIC

            /* compute the upward and downward  directional derivatives - LOIC */
            if (La_l1 != NULL){
                #pragma omp parallel for private(rv, s, t, v) num_threads(ntV_rV)
                for (rv = 0; rv < rV_; rv++){
                    if ((*rX)[rv] == ZERO){
                        /* run along the component rv */
                        for (v = Vc[s = rVc[rv]], t = rVc[rv+1]; s < t; v = Vc[++s]){
                            DfS_up[v] = DfS[v] + La_l1[v];
                            DfS_do[v] = DfS[v] - La_l1[v];
                        }
                    }else{
                        /* run along the component rv */
                        for (v = Vc[s = rVc[rv]], t = rVc[rv+1]; s < t; v = Vc[++s]){
            ...
            /* set the source/sink capacities - LOIC */
            for (v = 0; v < V; v++){
                m = MAX(0,MAX(- DfS_up[v], DfS_do[v]));
                G->nodes[v].tr_cap = - DfS_do[v] + m; //source to v1
                G->nodes[v + V].tr_cap = - (DfS_up[v] + m); // v2 to sink
                G->arcs[4 * E + 2 * v].r_cap = m; //v1 to v2
                G->arcs[4 * E + 2 * v + 1].r_cap = ZERO; //v2 to v1
            }
            /* set the d1 edge capacities - LOIC*/
            for (e = 0; e < E; e++){
                i = 4*e;
                if (G->arcs[i].is_active){
                    G->arcs[i].r_cap = ZERO;
                    G->arcs[i+1].r_cap = ZERO;
                    G->arcs[i+2].r_cap = ZERO;
                    G->arcs[i+3].r_cap = ZERO;
                }else{
                    a = La_d1[e];
                    G->arcs[i].r_cap = a;
                    G->arcs[i+1].r_cap = a;
            ...
            /*activate edges - LOIC*/
            #pragma omp parallel for private(e, i, a) reduction(+:w) num_threads(ntE)
            for (e = 0; e < E; e++){
                i = 4 * e;
                if (!G->arcs[i].is_active){
                    if ((G->what_segment(Eu[e]) != G->what_segment(Ev[e]))
                      ||(G->what_segment(Eu[e] + V) != G->what_segment(Ev[e] + V))){
                        //std::cout << Eu[e] << " to " << Ev[e] << " activated" << std::endl;
                        G->arcs[i].is_active = TRUE;
                        G->arcs[i + 1].is_active = TRUE;
                        //we only activate the edges on the first stage
                        w++;
                    }
                }
            }
        ...
        ntrVN = compute_num_threads(rV_*N, rV_);
        ...
        for (ru = 0; ru < rV_; ru++){
            ...
            for (u = Vc[s = rVc[ru]], t = rVc[ru+1]; s < t; u = Vc[++s]){
                ...
                for (ee = G->nodes[u].first; ee; ee = ee->next){
                    ...
                    e = ((int) (ee - G->arcs))/4; // LOIC
                    if (e > E) { continue; } // is a vertical v1 -> v2 edge - LOIC
                    a = La_d1[e];
                    if (a == ZERO){ continue; }
                    i = 0; /* a nonzero edge involving ru exists */
                    v = (int) (ee->head - G->nodes);
                    if (v > V){ std::cout << "SHOULD NEVER HAPPEN 2" << std::endl;} // LOIC
                    rv = Cv[v];
        ...
        if (preAt && N != 0){ /* fill lower triangular part */
            #pragma omp parallel for private(ru, rv, rAv, i) \
                schedule(static) num_threads(ntrVrV)
        ...
        *rX = (real*) CALLOC(rV_, sizeof(real));
        ...
        if (Obj != NULL){ 
            ...
            if (La_l1 != NULL){ /* ||x||_{l1,La_l1} */
                a = ZERO;
                #pragma omp parallel for private(rv, b) reduction(+:a) \
                    schedule(static) num_threads(ntrV)
                for (rv = 0; rv < rV_; rv++){
                    b = (*rX)[rv];
                    if (b < ZERO){ b = -b; }
...
template void CP_PFDR_graph_quadratic_d1_l1_duplex<float>(const int, const int, \
...
template void CP_PFDR_graph_quadratic_d1_l1_duplex<double>(const int, const int, \
